...
def log_gc_state(label: str):
    counts = gc.get_count()          # (gen0, gen1, gen2) 누적 할당-해제 카운터
    thresh = gc.get_threshold()      # (gen0, gen1, gen2) 임계치
    plog(f"[GC] {label} counts={counts} thresholds={thresh}")

def enable_gc_tracing():
    if not getattr(enable_gc_tracing, "_enabled", False):
        def _gc_cb(phase, info):
            # phase: 'start' | 'stop', info: {'generation', 'collected', 'uncollectable'}
            plog(f"[GC] phase={phase} info={info}")
        gc.callbacks.append(_gc_cb)
        enable_gc_tracing._enabled = True
        plog("[GC] tracing enabled")

@provide_session
def _get_priority_files(test="a", asd="d", session: Session = NEW_SESSION) -> list[DagFileInfo]:
...
def child_process(child_id):
    """자식 프로세스에서 실행되는 함수"""
    enable_gc_tracing()
    log_gc_state("child start (pre-initial collect)")
    gc.collect()
    log_gc_state("child after initial gc.collect()")
    ...
    t1 = time.time()
    log_gc_state("after DagBag creation (pre-collect)")
    gc.collect()
    log_gc_state("after DagBag creation (post-collect)")

    plog(f"Child-{child_id} finished (DagBag {t1 - t0:.3f}s)")
...
def main(use_connection_ids=False, use_register_at_fork=True):
    ...
    #     "mysql+pymysql://root:mysqlrootpass@172.23.237.101/airflow",
    #     pool_size=3,
    #     max_overflow=2,
    #     pool_recycle=1800,
    ...
    enable_gc_tracing()
    tracemalloc.start()
    log_gc_state("parent before engine creation")
