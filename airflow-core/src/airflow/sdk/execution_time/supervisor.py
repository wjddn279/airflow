...
def _fork_main(
    ...
    plog(f"sub process start to run in os pid {os.getpid()}")
    logging.debug(f"sub process start to run in os pid {os.getpid()}")
    ...
    plog(f"ready to run in os pid {os.getpid()}")
    log.warning(f"ready to run in os pid {os.getpid()}")
    ...
    try:
        ...
        plog(f"target start to run in os pid {os.getpid()}")
        logging.debug(f"target start to run in os pid {os.getpid()}")
        target()
        plog(f"sub process exit in os pid {os.getpid()}")
        logging.debug(f"sub process exit in os pid {os.getpid()}")
        exit(0)
    except SystemExit as e:
        plog(f"SystemExit in os pid {os.getpid()}")
        logging.debug(f"SystemExit in os pid {os.getpid()}")
        ...
        plog(f"Exception in os pid {os.getpid()}")
        logging.debug(f"Exception in os pid {os.getpid()}")
        plog(e)
        ...
        try:
            ...
            plog(f"Supervised process Last chance exception handler  in os pid {os.getpid()}")
            logging.debug(f"Supervised process Last chance exception handler  in os pid {os.getpid()}")
