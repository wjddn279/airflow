...
def main(use_connection_ids=False, use_register_at_fork=True):
    ...
    def set_mysql_timezone(dbapi_connection, connection_record):
        ...
        cid = cursor.execute("SELECT CONNECTION_ID()")
        cursor.close()
        print(cid)
    ...
    @event.listens_for(engine1, "close")
    def on_close(dbapi_connection, connection_record):
        plog("mysql connectio close os_id: " + str(os.getpid()))
        plog("close connection_record")
